...
import { Download, MapIcon, Navigation, Flag } from "lucide-react";
...
export function RouteMap({ route }: RouteMapProps) {
  ...
  const center = useMemo(() => {
    ...
    const avgLat =
      coordinates.reduce(
        (sum: number, coord: { lat: number; lng: number }) => sum + coord.lat,
        0,
      ) / coordinates.length;
    const avgLng =
      coordinates.reduce(
        (sum: number, coord: { lat: number; lng: number }) => sum + coord.lng,
        0,
      ) / coordinates.length;
  ...
  const handleDownloadGPX = () => {
    ...
    try {
      ...
      const gpxContent = generateGPXFromCoordinates(
        coordinates,
        route.title,
        route.description || "",
      );
  ...
  const generateGPXFromCoordinates = (
...
<gpx version="1.1" creator="MotoTrip" xmlns="http://www.topografix.com/GPX/1/1">
  ...
  const handleOpenInGoogleMaps = () => {
    ...
    // pega alguns pontos intermediários
    const sampled = coordinates.filter(
      (_, i:number) => i % Math.ceil(coordinates.length / 8) === 0,
    );
        ...
        <div className="aspect-video bg-destructive/10 flex items-center justify-center">
          <div className="text-center space-y-3 p-8 max-w-md">
            ...
            <div className="text-xs text-muted-foreground space-y-2">
              ...
              <p className="pt-2">
                no arquivo <strong>.env.local</strong>
              </p>
            </div>
          </div>
        </div>
            ...
            >
              ...
              {coordinates.length > 0 && (
                <AdvancedMarker
                  ...
                  zIndex={20}
                >
                  <div className="p-2 bg-primary text-white m-auto rounded-full shadow-lg border-2">
                    <Navigation />
                  </div>
                </AdvancedMarker>
              )}
